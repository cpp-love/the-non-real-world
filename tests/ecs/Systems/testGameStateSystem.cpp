...
int main() {
    ...
    assert_msg(notcreated, "未创建游戏状态，退出");
    notcreated = GameStateSystem::createGameState(registry);
    assert_msg(!notcreated, "重复创建游戏状态，退出");
    ...
    assert_msg(succeeded, "加入状态失败，退出");
    ...
    assert_msg(state.has_value(), "获取状态失败，退出");
