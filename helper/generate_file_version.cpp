...
int main(int argc, char *argv[]) {
    ...
    if (!std::filesystem::is_directory(workspace_folder)) {
        fmt::println(u8":: 错误：工作区目录指向位置不是一个目录");
        return 0;
    }
    if (!std::filesystem::is_regular_file(file)) {
        fmt::println(u8"错误：文件路径指向位置不是文件");
        return 0;
    }
    ...
    if (!input) {
        fmt::println(stderr, u8":: 错误：打开模板文件（位置：{}）失败",
    ...
    std::string      formatted_string = fmt::vformat(
        file_string_view, fmt::make_format_args(file_name, date_string, authors));
